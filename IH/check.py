...
if __name__ == '__main__':
    ...
    reveal='./checkpoint/epoch_220_reveal.pkl'
    reveal_net=moxing(reveal)
    
    output='./result1/output.jpg'
    output=transforms_img(output)

    reveal_secret = reveal_net(output)

    secret = './result1/secret.jpg'
    secret = transforms_img(secret)
    # norm(secret, reveal_secret)

    save_image(reveal_secret.cpu().data[:4],fp='./result1/reveal_secret.jpg')
